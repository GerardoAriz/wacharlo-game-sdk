...
export class GameSDK {
    ...
    initialize() {
        ...
        this._logger.info(`@wacharlo/game-sdk v${SDK_VERSION} | "${this._config.gameSlug}" v${this._config.gameVersion} | ${this._transport.constructor.name}`);
        ...
        if (typeof window !== 'undefined') {
            ...
            const preloadedId = (globalWacha && typeof globalWacha.sessionId === 'string' && globalWacha.sessionId.trim() !== '')
                ...
                : (window.__WACHA_SESSION_ID__ && typeof window.__WACHA_SESSION_ID__ === 'string')
                    ? window.__WACHA_SESSION_ID__.trim()
                    : null;
            if (preloadedId) {
            ...
            if (eventName === 'ADOPT_SESSION') {
                ...
                if (id && typeof id === 'string' && id.trim() !== '') {
                    if (this._sessionActive) {
                        this._logger.error(`ADOPT_SESSION message received AFTER startSession() — running session cannot be modified. Provided ID "${id}" ignored.`);
                    }
                    else {
    ...
    adoptSessionId(sessionId, origin = 'manual') {
        if (!this._initialized) {
            this._logger.warn('adoptSessionId() called before initialize(). Call sdk.initialize() first. The session ID was NOT adopted.');
            return;
        }
        if (this._sessionActive) {
            this._logger.error(`adoptSessionId() called AFTER startSession() — the running session ID cannot be changed retroactively. ` +
    ...
    startSession() {
        ...
        this._session.start(this._hostSessionId ?? undefined, (this._hostSessionId && this._hostSessionOrigin !== 'none') ? this._hostSessionOrigin : 'generated');
        this._data.reset();
